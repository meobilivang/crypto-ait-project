...
def aes_enc_symkey(
    ...
    # sqn+rnd (in raw bytes form) will be used as the nounce
...
# # 1. generate a symkey 32-byte key
# symkey = get_random_bytes(32)
# print("Original symkey: ", symkey)
# print()

# # 2. encrypt symkey with pub key
# enc_symkey = rsa_enc_symkey(symkey)

# print("Encrypted symkey: ", enc_symkey)
# print("Len Encrypted symkey: ", len(enc_symkey))
# print()

# # 3. decrypt symkey with private key
# dec_symkey = rsa_dec_symkey(enc_symkey)
# print("Decrypted symkey: ", dec_symkey)
# print("Len Decrypted symkey: ", len(dec_symkey))
# print()

# # 4. encrypt a payload with sym key
# # sample 14-byte header: version, type, len, sqn
# ver = b"\x01\x00"
# type = b"\x03\x00"
# header_len = 14
# len = header_len.to_bytes(2, byteorder='big')
# sqn = 2
# sqn = sqn.to_bytes(2, byteorder='big')
# rnd = get_random_bytes(6)

# sample_header = ver + type + len + sqn + rnd

# hex_string = "48656c6c6f2c20576f726c64"
# # Convert to bytes
# hex_bytes = bytes.fromhex(hex_string)

# sample_payload = hex_bytes
# enc_payload, mac = aes_enc_symkey(header=sample_header, payload=sample_payload, aes_symkey=dec_symkey, header_sqn=sqn, header_rnd=rnd)

# print("Encrypted Payload: ", enc_payload, mac)

# # 5. decrypt a payload a symkey
# dec_payload = aes_dec_symkey(header=sample_header, encrypted_payload=enc_payload, mac=mac, aes_symkey=dec_symkey, header_sqn=sqn, header_rnd=rnd)
